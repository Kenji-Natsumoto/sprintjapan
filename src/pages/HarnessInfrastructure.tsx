...
const HarnessInfrastructure = () => {
  useEffect(() => {
    ...
    canonical.href = `${window.location.origin}/insights/harness-infrastructure/`;
  }, []);

  return (
    <SjShell>
      <InsightPaper id="harness-infrastructure" content={content} abstract="AI-Native Company を物理的・技術的に成立させる基礎インフラ設計。常時稼働性・遠隔監督性・データ統合性を満たす4層モデルと、既存運用を破壊しない段階的移行戦略を示す。" />
    </SjShell>
  );
};
